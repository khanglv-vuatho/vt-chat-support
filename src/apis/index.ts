...
const fetchMessage = async ({ orderId, worker_id, page }: { orderId: number; worker_id?: number; page: number }) => {
  ...
  const response = await instance.get(endpoint, { params: { ...params, page } })
