...
export const BackgroundBeamsWithCollision = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    <div
      ref={parentRef}
      className={cn(
        'relative size-full items-center justify-center overflow-hidden bg-gradient-to-b from-white to-neutral-100',
        // h-screen if you want bigger
        className
      )}
    >
      {beams.map((beam) => (
        <div key={beam.initialX + 'beam-idx'}>
          <CollisionMechanism beamOptions={beam} containerRef={containerRef} parentRef={parentRef} />
        </div>
      ))}
